...
/**
 * Class WooCommerce_Urb_It_Admin
 */
class WooCommerce_Urb_It_Admin extends WooCommerce_Urb_It
{
    ...
    {
        add_submenu_page('woocommerce', __('Urb-it', self::LANG), __('Urb-it', self::LANG), 'manage_woocommerce',
            $this->settings_url(false), '');
    }
    ...
    public function notices()
    {
        ...
        if ($screen->id !== 'woocommerce_page_wc-settings' || $_GET['tab'] !== 'urb_it') {
            if (!function_exists('curl_version')) {
                ...
                <div class="notice notice-error">
                    <p><?php _e('Urb-it requires cURL to work. Please contact your hosting provider or urb-it for help.',
                            self::LANG); ?></p>
                </div>
                <?php
                return;
            ...
            if (!$this->setting('store_key') || !$this->setting('shared_secret')) {
                ...
                <div class="notice notice-warning">
                    <p><?php echo sprintf(__('You have to <a href="%s">set up some settings</a> before your customers can use urb-it\'s shipping methods.',
                            self::LANG), $this->settings_url()); ?></p>
                </div>
                <?php
            }
        }
        ...
        if ($this->setting('environment') === 'prod' && $this->setting('log') === 'everything') {
            ?>
            <div class="notice notice-warning">
                <p><?php echo sprintf(__('You are in production environment and are currently logging everything, which isn\'t recommended. Please set the logging option to "%s" in <a href="%s">the settings</a>.',
                        self::LANG), __('Only errors', self::LANG), $this->settings_url()); ?></p>
            </div>
            <?php
        }
    }
    ...
    public function bulky_product_field()
    {
        woocommerce_wp_checkbox(array(
            'id'          => '_urb_it_bulky', 'label' => __('Bulky Product', self::LANG),
    ...
    public function bulky_product_field_save($post_id)
    {
        ...
        if (!empty($bulky)) {
            update_post_meta($post_id, '_urb_it_bulky', $bulky);
        } else {
            delete_post_meta($post_id, '_urb_it_bulky');
        }
    }
    ...
    {
        ?><p
            class="form-field"><?php echo sprintf(__('Note: If the product exceeds %d kilos and/or %d liters, it can\'t be delivered by urb-it.',
        self::LANG), self::ORDER_MAX_WEIGHT, self::ORDER_MAX_VOLUME / 1000); ?></p><?php
    }
