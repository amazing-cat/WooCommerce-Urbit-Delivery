...
class WooCommerce_Urb_It_Coupon extends WooCommerce_Urb_It
{
    ...
    public function options_view()
    {
        woocommerce_wp_checkbox(array(
            ...
            'description' => __('Check this box if the coupon only should be valid when urb-it is selected as shipping method.',
                self::LANG),
        ));
        ...
        woocommerce_wp_checkbox(array(
            ...
            'description' => __('Check this box if the coupon grants free shipping with urb-it. This will remove the costs for the urb-it shipping methods when the coupon is applied.',
                self::LANG),
        ));
    }
    ...
    public function populate($coupon)
    {
        ...
        if (!isset($coupon->urb_it_free_shipping)) {
            $coupon->urb_it_free_shipping = get_post_meta($coupon->id, 'urb_it_free_shipping', true);
        }
        if (!isset($coupon->urb_it_only)) {
            $coupon->urb_it_only = get_post_meta($coupon->id, 'urb_it_only', true);
        }
    }
    ...
    public function set_shipping_cost($cost)
    {
        if ($coupons = wc()->cart->get_coupons()) {
            foreach ($coupons as $code => $coupon) {
                if ($coupon->is_valid() && $coupon->urb_it_free_shipping === 'yes') {
                    return 0;
                }
            }
        }
    ...
    public function validate_coupon($is_valid, $coupon)
    {
        ...
        foreach ($shipping_methods as $shipping_method) {
            if (strncmp('urb_it_', $shipping_method, 7) === 0) {
                return true;
            }
        }
    ...
    {
        if ($coupon->urb_it_only !== 'yes' || $msg_code !== $coupon::WC_COUPON_SUCCESS) {
            return $msg;
        }

        return __('Awesome! Ensure you\'re selecting urb-it as shipping method during checkout, and the discount is yours.',
            self::LANG);
    }
}
