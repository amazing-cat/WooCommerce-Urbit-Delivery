...
/**
 * Class WooCommerce_Urb_It_Frontend
 */
class WooCommerce_Urb_It_Frontend extends WooCommerce_Urb_It
{
    ...
    {
        if ($this->setting('notice_product_page') !== 'yes') {
            return;
        }

        global $product;

        if ($this->validate->product_volume($product) && $this->validate->product_weight($product)) {
            return;
        }
    ...
    public function notice_added_product(
        $cart_item_key,
        $product_id,
        $quantity,
        $variation_id,
        ...
        if ($this->setting('notice_added_product') !== 'yes') {
            return;
        }
        ...
        if (!$this->validate->cart_weight()) {
            wc_add_notice(sprintf(__('Your cart\'s total weight exceeds %d kilos and can\'t be delivered by urb-it.',
                self::LANG), self::ORDER_MAX_WEIGHT), 'notice');
        } elseif (!$this->validate->cart_volume()) {
            wc_add_notice(sprintf(__('Your cart\'s total volume exceeds %d liters and can\'t be delivered by urb-it.',
                self::LANG), self::ORDER_MAX_VOLUME / 1000), 'notice');
        } elseif (!$this->validate->cart_bulkiness()) {
            wc_add_notice(__('Your cart contains a bulky product and can\'t be delivered by urb-it.', self::LANG),
                'notice');
        }
    }
    ...
    public function shipping_calculator()
    {
        ...
        if (!in_array('urb_it_specific_time', $shipping_method, true) && !in_array('urb_it_one_hour', $shipping_method,
                true)) {
            return;
        }

        if (!empty($_POST['urb_it_date']) && !empty($_POST['urb_it_hour']) && !empty($_POST['urb_it_minute'])) {
            wc()->session->set('urb_it_delivery_time',
                esc_attr($_POST['urb_it_date']) . ' ' . esc_attr($_POST['urb_it_hour']) . ':' . esc_attr($_POST['urb_it_minute']));
        }
        ...
        ?><p><input class="button" type="submit" name="calc_shipping" value="<?php _e('Save', self::LANG); ?>"/>
        </p><?php
        ?></form><?php
    }
}
